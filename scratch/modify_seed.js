...
for (const up of updates) {
  const matchStr = `uid: "${up.uid}",`;
  if (content.includes(matchStr)) {
    const insertStr = `${matchStr}\n        logoUrl: "${up.logoUrl}",\n        city: "${up.city}",\n        suburb: "${up.suburb}",\n        description: "${up.description}",`;
    content = content.replace(matchStr, insertStr);
  }
}

// Now replace the placehold.co part
const placeholdRegex = /const allProducts = \[\.\.\.faceprintProducts, \.\.\.featuredProducts\]\.map\(p => \{\s*const getImg = \(name\) => `https:\/\/placehold\.co\/600x400\/1a1c23\/06b6d4\?font=Montserrat&text=\$\{encodeURIComponent\(name\)\.replace\(\/%20\/g, \'\+\'\)\}`;/;
...
content = content.replace(placeholdRegex, imgMapCode);
content = content.replace(/imageUrl: getImg\(p.name\),/g, "imageUrl: imgMap[p.name] || defaultImg,");
